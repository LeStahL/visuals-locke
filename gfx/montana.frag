...
bool ray(out vec3 x, inout float d, vec3 dir, out SceneData s, vec3 o, vec3 l, out vec3 n)
{
    for(int i=0; i<250; ++i)
    {
        x = o + d * dir;
        ...
        d += s.dist<1.e-2?min(s.dist,.1*dz):min(s.dist,.2*dz);
    }
    return false;
}
